...
export default function PaymentsPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  useEffect(() => {
    params.then((p) => {
      console.log('🔍 Payments Page: Fetching analytics for company:', p.companyId)
      ...
      Promise.all([
        ...
        .catch(err => {
          console.error('❌ Payments Page: Failed to fetch data:', err)
          setLoading(false)
        })
    })
  }, [params])
  ...
  const {
    ...
    aggregation,
    setAggregation,
  } = useChartData(historicalData, 'payments', 'successful')
